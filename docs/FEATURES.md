# Features Checklist

อัปเดตสถานะ `[ ]` -> `[x]` เมื่อทำเสร็จ เพื่อให้ AI/ทีมรู้ progress ปัจจุบัน

## 3.1 ระบบยืนยันตัวตน (Login - Logout)
- [x] Login สำหรับ Admin + Leader (JWT, bcrypt)
- [x] เช็ค session ปัจจุบัน (/me)
- [ ] จัดการบัญชีผู้ใช้งาน (Admin เท่านั้น)
- [ ] แก้ไข username/password ของตนเอง
- [ ] LINE LIFF auto-register/verify (ลูกบ้าน, ดึง LINE User ID อัตโนมัติ)

## 3.2 ระบบจัดการข่าวสารและประกาศหมู่บ้าน
- [x] เพิ่มข่าว (Admin, Leader เสนอ/ร่าง)
- [x] แก้ไขข่าว (Admin ทุกข่าว / Leader เฉพาะที่ตนสร้างและยังไม่อนุมัติ)
- [x] ลบข่าว (Admin เท่านั้น)
- [x] แนบไฟล์/รูปภาพประกอบข่าว
- [x] สถานะข่าว: ร่าง / รออนุมัติ / อนุมัติ / เผยแพร่แล้ว

## 3.3 ระบบอนุมัติข่าวก่อนเผยแพร่
- [x] ตรวจสอบข่าวก่อนส่ง (GET /api/news/:id)
- [x] อนุมัติข่าว (Admin)
- [x] ยกเลิกการเผยแพร่ข่าว (Admin) — ใช้ reject endpoint

## 3.4 ระบบจัดหมวดหมู่ข่าวสาร
- [x] เพิ่ม/แก้ไข/ลบ หมวดหมู่ (Admin)
- [x] กำหนดหมวดหมู่ให้ข่าวแต่ละรายการ

## 3.5 ระบบปฏิทินกิจกรรมของชุมชน
- [ ] เพิ่ม/แก้ไขกิจกรรม (Admin, Leader)
- [ ] แสดงกิจกรรมผ่าน LINE OA ให้ลูกบ้านดู

## 3.6 ระบบดาวน์โหลดแบบฟอร์มราชการ
- [ ] อัปโหลด/แก้ไข/ลบไฟล์ฟอร์ม (Admin)
- [ ] ดาวน์โหลดฟอร์มผ่าน LINE OA (ลูกบ้าน)

## 3.7 ระบบส่งข่าวผ่าน LINE Official Account
- [ ] เลือกกลุ่มผู้รับข่าว
- [ ] ส่งข่าวแบบลิงก์/รูปภาพผ่าน LINE Messaging API
- [ ] ตั้งเวลาส่งข่าวล่วงหน้า

## 3.8 ระบบแชทบอทตอบคำถามอัตโนมัติ
- [ ] ตอบคำถามพื้นฐานอัตโนมัติ (keyword match กับ tb_chatbot_faq)
- [ ] ส่งลิงก์ข่าว/ฟอร์มที่เกี่ยวข้อง
- [ ] บันทึกประวัติการสอบถาม
- [ ] fallback "ไม่พบข้อมูล" + แจ้งเตือน Admin ให้ตอบ 1-on-1

## 3.9 ระบบบันทึกประวัติการส่งข่าว (Broadcast Log)
- [ ] บันทึกวันที่/เวลา/ผู้ส่ง/จำนวนผู้รับ

## 3.10 ระบบรายงานและสถิติการเข้าถึงข้อมูล
- [ ] จำนวนข่าวที่เผยแพร่
- [ ] สถิติการเข้าถึงข่าว (Tracking Views ผ่าน tb_view_log)
- [ ] รายงานสรุปย้อนหลัง

---

## Infrastructure / Setup (ทำก่อนเริ่มฟีเจอร์ด้านบน)
- [x] ออกแบบโครงสร้างโปรเจกต์ (backend + frontend)
- [x] สร้าง Messaging API Channel + LIFF Channel (LINE Developers Console)
- [ ] ตั้งค่า Webhook URL + verify
- [ ] ปิด auto-reply/greeting message default ใน LINE OA Manager
- [ ] รัน schema.sql สร้างฐานข้อมูลจริง
- [ ] เขียน middleware auth (JWT) + role check

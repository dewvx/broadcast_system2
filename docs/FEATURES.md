...
- [x] เชื่อมโยงกิจกรรมชุมชนเข้ากับข่าวสาร (Linked Activity) — เลือกกิจกรรมในปฏิทินแนบกับข่าวได้
- [x] แสดงการ์ดกิจกรรมที่เกี่ยวข้องในหน้าอ่านข่าวของลูกบ้าน (LIFF `/liff/news/:id`) พร้อมปุ่มเปิดดูกิจกรรมในปฏิทิน
...
- [x] มุมมองปฏิทินรายเดือนแบบ Interactive (Month Calendar Grid) สลับมุมมองกับรายการการ์ด (ปฏิทิน / รายการ)
- [x] คัดลอกลิงก์กิจกรรม (Copy LIFF URL) ได้คลิกเดียวจากการ์ดกิจกรรมและปฏิทิน เพื่อส่งต่อใน LINE
...
- [x] fallback "ไม่พบข้อมูล" พร้อมระบบให้แอดมินตอบกลับลูกบ้าน 1-on-1 เข้า LINE ส่วนตัวผ่าน Web Dashboard (`POST /api/chatbot-faq/logs/:id/reply`)
...
## 3.13 ระบบจัดการซอย/คุ้ม (Zone Management)
> **หมายเหตุ Scope:** ระบบใช้งาน 1 หมู่บ้านต่อ 1 ชุดระบบ (1 database) ไม่รองรับ multi-tenancy โดยคำว่า "โซน" เดิมเปลี่ยนเป็น "ซอย/คุ้ม" ภายในหมู่บ้านเดียวกัน
- [x] ตาราง `tb_zone` พร้อม CRUD Backend (`/api/zone`)
- [x] Dropdown ซอย/คุ้มบนหน้าลงทะเบียนและแก้ไขข้อมูลลูกบ้าน (LIFF)
- [x] Server-side validation เช็ค `zoneName` กับ `tb_zone` (Reject HTTP 400 หากไม่ถูกต้อง)
- [x] หน้าจัดการซอย/คุ้มสำหรับ Admin (`/admin/zones`)
- [x] เลือกผู้รับข่าวแบบเจาะจง ซอย/คุ้ม ใน Broadcast

## 3.14 ระบบการ์ดผู้ติดต่อผู้นำชุมชนแบบ Dynamic (Dynamic Leader Contacts)
- [x] คอลัมน์ `phone_number` และ `position_title` ใน `tb_user`
- [x] Public Endpoint `GET /api/user/public/contacts` (ดึงเฉพาะชื่อ, เบอร์โทร, ตำแหน่ง สำหรับผู้ใช้ที่มีเบอร์โทร โดยไม่หลุดข้อมูล sensitive)
- [x] การ์ดผู้ติดต่อผู้นำชุมชนแบบ Dynamic บนหน้าโปรไฟล์ลูกบ้าน (LIFF `/liff/profile`) พร้อมปุ่มกดโทรออกทันที
